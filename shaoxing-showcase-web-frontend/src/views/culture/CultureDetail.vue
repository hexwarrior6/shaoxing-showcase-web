<template>
  <Header/>
  <div class="culture-detail-container" v-if="culture">
    <div class="culture-header">
      <h1 class="culture-name">{{ culture.cultureName }}</h1>
      <p class="culture-time">{{ culture.time }}</p>
    </div>
    ...
    <div class="culture-description">
      ...
      <h2 class="indent-title">意义</h2>
      <p>{{ culture.significance }}</p>
    </div>

    <!-- 底部按钮 -->
    <div class="button-container">
      <el-button style="font-size: larger; padding: 20px" type="primary" plain @click="editCulture(culture.id)">
...
export default {
  ...
  setup() {
    ...
    const editCulture = (cultureId) => {
      window.location.href = `/culture/edit/${cultureId}`
    }

    const confirmDelete = () => {
      ElMessageBox.confirm('确定要删除这项文化吗？', '警告', {
    ...
    return {culture, editCulture, confirmDelete};
  }
};
</script>
